...
class AllEmployeesKppService {
    ...
    getEmployeeByStatusByPagination(empKppStaus) {
        if (null != Cookies.get('empId')) {
            ...
            return axios.get(`http://localhost:9091/employee/employee-kpp-status?gmEmployeedId=${Cookies.get('empId')}&roleId=1&empKppStatus=${empKppStaus}&page=0&size=1200&sort=desig.desig.name`)
        } else {
            alert("You need to login first")
            window.location.replace("http://localhost:3008");
        }
